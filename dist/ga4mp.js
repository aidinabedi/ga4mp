...
var sendRequest = function sendRequest(endpoint, payload) {
  ...
  if (mode === 'browser') {
    ...
    var req = scheme === 'https' ? require('https') : require('http');
    var options = {
      headers: {
...
var ga4mp = function ga4mp(measurement_ids) {
  ...
  if (!measurement_ids) throw new Error('Tracker initialization aborted: missing tracking ids');
      ...
      if (payload && forceDispatch) {
        for (var i = 0; i < payload.tid.length; i++) {
          ...
          sendRequest(internalModel.endpoint, r, internalModel.mode, {
            ...
            encode_search_params: internalModel === null || internalModel === void 0 ? void 0 : internalModel.encode_search_params
          });
        }
